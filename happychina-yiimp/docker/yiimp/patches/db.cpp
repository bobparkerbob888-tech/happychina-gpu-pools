...
void db_query(YAAMP_DB *db, const char *format, ...)
{
	...
	while(!g_exiting)
	{
		...
		if(res == ER_LOCK_DEADLOCK)
			stratumlog("SQL DEADLOCK %d/%d: %s | %.220s\n", attempt, max_retries, mysql_error(&db->mysql), buffer);
		else
		...
		if (attempt >= max_retries) {
			if(res == ER_LOCK_DEADLOCK) {
				stratumlog("SQL DEADLOCK dropping query after %d attempts: %.220s\n", attempt, buffer);
				break;
			}
			exit(1);
		}
		...
		int sleep_ms = 100;
		if(res == ER_LOCK_DEADLOCK) {
			sleep_ms = attempt * 200;
			if(sleep_ms > 1000) sleep_ms = 1000;
		}
		usleep(sleep_ms*YAAMP_MS);
