...
{
	int paramlen = strlen(block);
	bool use_submitauxblock =
		(strcmp(coind->symbol, "XMY") == 0) || (strcmp(coind->symbol2, "XMY") == 0) ||
		(strcmp(coind->symbol, "QBC") == 0) || (strcmp(coind->symbol2, "QBC") == 0) ||
		(strcmp(coind->symbol, "LCN") == 0);
	const char *method = use_submitauxblock ? "submitauxblock" : "getauxblock";
	...
	json = rpc_call(&coind->rpc, method, params, coind);
	...
	{
		const char *p = json_get_string(json_error, "message");
		if(p && strstr(p, "block hash unknown"))
		{
			stratumlog("STALE AUX %s %s (%s)\n", coind->name, p, method);
			json_value_free(json);
			coind_getauxblock(coind);
			return false;
		}
		if(p) stratumlog("ERROR %s %s (%s)\n", coind->name, p, method);
