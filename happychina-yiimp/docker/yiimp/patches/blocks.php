...
function BackendRunShareRewardUpdate($sql, $params, $context)
{
	$max_attempts = 5;
	...
	for($attempt = 1; $attempt <= $max_attempts; $attempt++)
	{
		try {
			...
			if(!$is_lock_error || $attempt >= $max_attempts) {
				debuglog("unable to update shares $context after $attempt attempts: $message");
				throw $e;
			}

			debuglog("unable to update shares $context attempt $attempt/$max_attempts: $message");
...
//	debuglog("NEW BLOCK $coin->name $db_block->height");
	...
	if (!$is_solo)
	{
		...
		foreach($list as $item)
		{
			...
			$last_shared = dborow("SELECT height, time FROM blocks " .
				"WHERE coin_id=:id AND solo=0 AND category IN ('immature','generate') AND id<:blockid " .
				"ORDER BY time DESC, id DESC LIMIT 1",
				array(':id' => $coin->id, ':blockid' => $db_block->id));
			$timelast_shared = (int) arraySafeVal($last_shared, 'time');

			$total_shared_difficulty = dboscalar(
				"SELECT SUM(difficulty) FROM shares WHERE coinid=:coinid AND algo=:algo AND solo=0 AND time>:time1 AND time<=:time2",
				array(':algo'=>$coin->algo, ':coinid'=>$coin->id, ':time1'=>$timelast_shared, ':time2'=>$db_block->time)
			);
	...
	else 
	{
		...
		$last_solo = dborow("SELECT height, time FROM blocks " .
			"WHERE coin_id=:id AND solo=1 AND category IN ('immature','generate') AND id<:blockid " .
			"ORDER BY time DESC, id DESC LIMIT 1",
			array(':id' => $coin->id, ':blockid' => $db_block->id));
		$timelast_solo = (int) arraySafeVal($last_solo, 'time');

		$total_solo_difficulty = dboscalar(
			"SELECT SUM(difficulty) FROM shares WHERE coinid=:coinid AND algo=:algo AND solo=1 AND time>:time1 AND time<=:time2",
			array(':algo'=>$coin->algo, ':coinid'=>$coin->id, ':time1'=>$timelast_solo, ':time2'=>$db_block->time)
		);
	...
	$shareUpdateSql =
		"UPDATE shares SET pid=-1, blocknumber=:height, blockrewarded=:height ".
		"WHERE coinid=:coinid AND algo=:algo AND blockrewarded IS NULL AND blocknumber <= :height ".
		(($is_solo) ? "AND solo=1" : "AND valid=1 AND solo!=1");

	BackendRunShareRewardUpdate($shareUpdateSql, array(
