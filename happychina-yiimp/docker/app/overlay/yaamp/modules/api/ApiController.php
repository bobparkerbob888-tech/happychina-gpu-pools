...
class ApiController extends CommonController
{
    ...
    public function actionStatus()
    {
        ...
        foreach (yaamp_get_algos() as $i => $algo) {
            ...
            $pool_hash = yaamp_pool_rate($algo) ? yaamp_pool_rate($algo) : '0';
            $pool_shared_hash = yaamp_pool_shared_rate($algo) ? yaamp_pool_shared_rate($algo) : '0';
            $pool_solo_hash = yaamp_pool_solo_rate($algo) ? yaamp_pool_solo_rate($algo) : '0';

            if ($algo === 'scrypt') {
                $parentCoin = getdbosql('db_coins', "algo=:algo AND symbol=:symbol AND enable", array(
                    ':algo' => $algo,
                    ':symbol' => 'LTC',
                ));
                if ($parentCoin) {
                    $pool_hash = yaamp_coin_rate($parentCoin->id) ? yaamp_coin_rate($parentCoin->id) : '0';
                    $pool_shared_hash = yaamp_coin_shared_rate($parentCoin->id) ? yaamp_coin_shared_rate($parentCoin->id) : '0';
                    $pool_solo_hash = yaamp_coin_solo_rate($parentCoin->id) ? yaamp_coin_solo_rate($parentCoin->id) : '0';
                }
            }

            $last_share_time = (int) controller()->memcache->get_database_scalar("api_status_lastshare-$algo", "select max(time) from shares where valid and algo=:algo", array(
                ':algo' => $algo
            ));
            $rate_window = (int) yaamp_hashrate_step();
            $last_share_age = $last_share_time > 0 ? max(0, time() - $last_share_time) : 0;
